...
public validate(){
  ...
  if(this.password && this.password.length < 4) throw new ValidationError("Password must be four or more characters");
  if ( !emailRegex.test(this.email)) {
    throw new ValidationError("Invalid email format.");
  }
  if(!this.googleId && !this.password){
    throw new ValidationError ("Missing password or google id.")
  }
}
}
